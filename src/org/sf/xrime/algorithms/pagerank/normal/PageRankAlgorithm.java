...
import org.matt.postprocessing.PageRankFormatter;
import org.sf.xrime.ProcessorExecutionException;
import org.sf.xrime.algorithms.GraphAlgorithm;
import org.sf.xrime.algorithms.BFS.alg_1.BFSStep;
...
public class PageRankAlgorithm extends GraphAlgorithm {
  ...
  public void execute() throws ProcessorExecutionException {
    try {
      ...
      /* Matt Edited Code */
      Path lastOutput = new Path(context.getDestination().getPath().toUri().toString() + String.valueOf(tempDirs.getSeqNum()-1));
      Path finalOutput = new Path(context.getDestination().getPath().toUri().toString() + "FINAL");
      
      PageRankFormatter converter = new PageRankFormatter();
      converter.setMapperNum(getMapperNum());
      		    
      try {
    	String[] args = {lastOutput.toString(), finalOutput.toString()};
        int res = ToolRunner.run(converter, args);
        if (res != 0)
        	throw new Exception("Didn't Print Out Good Format Correctly, error code: " + res);
      } catch (IOException e) {
